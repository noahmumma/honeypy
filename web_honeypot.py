...
if not funnel_logger.handlers:
    funnel_handler = RotatingFileHandler(
        'http_audits.log', maxBytes=2000, backupCount=5
    )
...
def web_honeypot(input_username="admin", input_password="password"):
    ...
    def login():
        ...
        funnel_logger.info(
            f'Client with IP Address: {ip_address} entered/n Username: {username}, Password: {password}')

        if username == input_username and password == input.password:
            return 'Have you ever seen The Mummy?'
        else:
